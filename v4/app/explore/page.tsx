        ...
        <section className="grid flex-1 grid-cols-1 lg:grid-cols-2 xl:grid-cols-3">
          {TRACKS.map((t, i) => (
            <Link
              key={t.id}
              href="/workspace"
              className={`ease-signature group relative flex flex-col gap-6 border-white/[0.04] px-10 py-10 transition-colors duration-300 hover:bg-white/[0.015] ${
                i % 3 !== 2 ? "xl:border-r" : ""
              } ${i % 2 === 0 ? "lg:border-r" : ""} border-b`}
            >
              <div className="flex items-center justify-between">
                ...
                <ArrowUpRight
                  className="h-4 w-4 text-zinc-700 transition-all group-hover:-translate-y-0.5 group-hover:translate-x-0.5 group-hover:text-[var(--signal)]"
                  strokeWidth={1.5}
                />
              </div>
