import { heroMetrics } from "@/lib/app-data"
import { ArrowUpRight, Minus } from "lucide-react"

export function MetricsRow() {
  return (
    <section className="grid border-b border-white/[0.04] md:grid-cols-2 lg:grid-cols-4">
      {heroMetrics.map((m, i) => (
        <div
          key={m.label}
          className={`relative flex flex-col justify-between gap-6 px-8 py-9 transition-colors duration-300 ease-[cubic-bezier(0.23,1,0.32,1)] hover:bg-white/[0.015] ${
            i < heroMetrics.length - 1 ? "lg:border-r lg:border-white/[0.04]" : ""
          } ${i < 2 ? "md:border-r md:border-white/[0.04]" : ""} ${
        ...
        >
          ...
          <div className="flex items-start justify-between">
            ...
            <span className="flex h-5 w-5 items-center justify-center border border-white/[0.06] text-zinc-600">
              {m.deltaTone === "up" ? (
                <ArrowUpRight className="h-2.5 w-2.5 text-[var(--signal)]/80" strokeWidth={1.5} />
              ) : (
          ...
          <div className="flex items-baseline gap-2">
            ...
            {m.unit && (
              <span className="font-mono text-[12px] tracking-widest text-zinc-600">
                {m.unit}
              </span>
            )}
          </div>
            ...
            <div className="flex items-center gap-2">
              <span
                className={`font-mono text-[11px] tracking-wider tabular-nums ${
                  m.deltaTone === "up" ? "text-[var(--signal)]" : "text-zinc-400"
                }`}
              >
                {m.delta}
              </span>
...
function MiniSpark({ index }: { index: number }) {
  // deterministic 8-point sparkline per metric
  const seeds = [
    [3, 5, 4, 6, 5, 7, 6, 8],
    [2, 3, 4, 4, 5, 5, 6, 7],
    [5, 4, 6, 5, 7, 6, 8, 7],
