import { skills } from "@/lib/app-data"
...
export function SkillsRadar() {
  ...
  // compute vertex for (i-th skill, value 0..100)
  ...
  const cohortPoints = skills.map((_, i) => vertex(i, 68 + ((i * 7) % 9))) // subtle comparator
  ...
  return (
    <div className="relative flex h-full flex-col px-10 py-9">
      <div className="mb-8 flex items-end justify-between">
        <div>
          ...
          <h2 className="mt-2 font-sans text-[26px] font-medium leading-tight tracking-tight-editorial text-zinc-100 text-balance">
            Mastery across the{" "}
            <span className="text-[var(--signal)]">six pillars</span>.
          </h2>
        </div>
      ...
      {/* legend table */}
      <div className="mt-6 grid grid-cols-6 gap-4 border-t border-white/[0.04] pt-4">
        {skills.map((s) => (
