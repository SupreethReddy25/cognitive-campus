...
export function RightPane({
  ...
  if (!open) {
    return (
      <aside className="relative flex min-h-0 flex-col items-center border-l border-white/[0.04] bg-white/[0.008]">
        <button
          onClick={() => onOpenChange(true)}
          className="ease-signature press group flex h-full w-full flex-col items-center justify-between py-3 transition-colors hover:bg-white/[0.02]"
          aria-label="Expand Socratic Mentor"
        >
          <div className="flex flex-col items-center gap-2.5">
            <div className="flex h-7 w-7 items-center justify-center border border-white/[0.06] bg-white/[0.01] transition-colors group-hover:border-white/[0.12]">
              <ChevronLeft className="h-3 w-3 text-zinc-500 group-hover:text-[var(--signal)]" strokeWidth={1.5} />
            </div>
            <span className="relative flex items-center justify-center">
              <Sparkles
                className={`h-3 w-3 ${lighthouse ? "text-[var(--signal)]" : "text-zinc-600"}`}
                strokeWidth={1.5}
              />
              {lighthouse && (
                <span className="status-dot absolute -right-1 -top-1 h-1 w-1 rounded-full bg-[var(--signal)]" />
              )}
            </span>
          </div>

          <span className="vlabel font-mono text-[10px] font-medium tracking-[0.28em] text-zinc-400 group-hover:signal-glow">
            MENTOR · 04
          </span>

          <div className="flex flex-col items-center gap-2 font-mono text-[9px] tracking-[0.2em] text-zinc-700">
            <span className={lighthouse ? "text-[var(--signal)]/70" : ""}>
              {lighthouse ? "ON" : "OFF"}
            </span>
            <span className="h-3 w-px bg-white/[0.06]" />
            <PanelRight className="h-3 w-3 text-zinc-700" strokeWidth={1.5} />
          </div>
        </button>

        <span className="absolute left-[-1.5px] top-4 h-[3px] w-[3px] rounded-[1px] bg-[var(--signal)] opacity-50" />
      </aside>
    )
  }
  ...
  return (
    <aside className="relative flex min-h-0 flex-col border-l border-white/[0.04]">
      ...
      <div
        ...
        className={`group absolute left-0 top-0 z-20 h-full w-1 cursor-col-resize hover:bg-[var(--signal)]/30 ${
          dragging ? "bg-[var(--signal)]/60" : ""
        }`}
      >
        <div className="absolute left-0 top-1/2 h-8 w-[2px] -translate-y-1/2 bg-white/[0.06] transition-colors group-hover:bg-[var(--signal)]/80" />
      </div>
      ...
      {/* Header */}
      <div className="flex h-9 shrink-0 items-center justify-between border-b border-white/[0.04] pl-5 pr-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="press ease-signature flex h-5 w-5 items-center justify-center text-zinc-600 hover:text-zinc-300"
            aria-label="Collapse"
          >
...
function MentorBody({ onCite }: { onCite: (lines: number[]) => void }) {
