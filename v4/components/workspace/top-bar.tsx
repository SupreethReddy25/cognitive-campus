...
export function TopBar({
  ...
  return (
    <header className="relative flex h-[44px] items-center justify-between px-4">
      {/* Left — breadcrumb + problem id + difficulty */}
      <div className="flex items-center gap-3 font-mono text-[11px] tracking-wider">
        {problem.breadcrumb.map((seg, i) => (
          <span key={seg} className="flex items-center gap-3">
            <span
              className={
                i === problem.breadcrumb.length - 1 ? "text-zinc-300" : "text-zinc-600"
              }
            >
              {seg}
            </span>
            {i < problem.breadcrumb.length - 1 && (
              <ChevronRight className="h-3 w-3 text-zinc-800" strokeWidth={1.5} />
            )}
          </span>
        ))}
      ...
      <div className="absolute left-1/2 top-1/2 flex -translate-x-1/2 -translate-y-1/2 items-center gap-3">
        ...
        <div className="relative h-[3px] w-40 overflow-hidden bg-white/[0.05]">
          <div
            className="absolute inset-y-0 left-0 bg-[var(--signal)] transition-[width] duration-700 ease-[cubic-bezier(0.23,1,0.32,1)]"
            style={{ width: `${bkt}%` }}
          />
          {[25, 50, 75].map((t) => (
            <span
