...
export function ArenaLobby() {
  ...
  return (
    <Page>
      <header className="flex flex-wrap items-start justify-between gap-6 pt-2 md:pt-6">
        <div>
          <div className="tag">Arena · real-time multiplayer</div>
          <h1 className="display mt-5 text-[clamp(42px,6.8vw,96px)] text-zinc-50">The <em className="text-[var(--ember)]">arena</em>.</h1>
        ...
        <div className="flex items-center gap-8 pt-2 text-right">
          {arenaRating && <div><div className="display text-[51.2px] leading-none tnum text-zinc-50">{arenaRating.elo}</div><div className="tag mt-1">{arenaRating.rank} rating</div></div>}
      ...
      {showWaitingRoom ? (
        <div className="mx-auto max-w-xl pt-20 text-center">
          ...
          <div className="tag mt-10">Room code</div>
          <div className="display mt-2 text-[clamp(58px,11.9vw,112px)] leading-none tracking-[0.12em] text-zinc-50">{roomCode}</div>
          <button onClick={copyCode} className="mt-4 inline-flex items-center gap-2 rounded-sm border border-[var(--line-strong)] px-5 py-2 font-mono text-[10.5px] uppercase tracking-[0.16em] text-zinc-400 transition-colors hover:border-[var(--ember)] hover:text-[var(--ember)]">{copied ? <Check className="h-4 w-4 text-emerald-400" /> : <Copy className="h-4 w-4" />}{copied ? 'Copied' : 'Copy code'}</button>
          <ul className="mx-auto mt-12 max-w-sm text-left">
            {players.map((p, i) => (
              <li key={p.userId || i} className="flex items-center gap-4 border-b border-[var(--line)] py-4">
                <span className="flex h-10 w-10 items-center justify-center rounded-full bg-[var(--ember)] text-[14px] font-semibold text-[#04130d]">{p.name?.[0]?.toUpperCase() || '?'}</span>
                <span className="display text-[20.8px] text-zinc-100">{p.name}</span>
        ...
        <div className="pt-16">
          <div className="mb-12 flex gap-1 border-b border-[var(--line-strong)] pb-3">
            {[['create', 'Create a match'], ['join', 'Join with a code']].map(([k, l]) => (
              <button key={k} onClick={() => setTab(k)} className={cn('relative rounded-sm px-5 py-2 font-mono text-[10.5px] uppercase tracking-[0.16em] transition-colors', tab === k ? 'text-zinc-50' : 'text-zinc-500 hover:text-zinc-200')}>
                {tab === k && <motion.span layoutId="arena-tab" className="absolute inset-0 rounded-sm bg-white/[0.08]" transition={{ type: 'spring', stiffness: 420, damping: 34 }} />}
                ...
                {MODES.map((m) => {
                  ...
                  return (
                    <button key={m.id} onClick={() => setSelectedMode(m.id)} className="group relative flex w-full items-baseline gap-6 border-b border-[var(--line)] py-6 text-left">
                      <span className="display w-12 text-[24px] leading-none text-zinc-700">{m.n}</span>
                      <span className={cn('display text-[clamp(30px,4.2vw,54px)] leading-none transition-all duration-300', active ? 'translate-x-2 text-[var(--ember)]' : 'text-zinc-300 group-hover:text-zinc-100')}>{m.label}</span>
                ...
                <div className="relative">
                  <div className="flex items-center gap-4 border-b border-[var(--line-strong)] pb-3 focus-within:border-[var(--ember)]">
                    <input value={searchQuery} onChange={(e) => { setSearchQuery(e.target.value); setSearchOpen(true); }} onFocus={() => setSearchOpen(true)} placeholder={selectedProblem ? selectedProblem.title : 'Search by name, difficulty or skill…'}
                      className={cn('display w-full bg-transparent text-[27.2px] outline-none placeholder:text-zinc-700', selectedProblem ? 'placeholder:text-[var(--ember-soft)]' : '')} />
                  ...
                  {searchOpen && (
                    <div className="absolute left-0 right-0 top-full z-20 mt-2 max-h-[340px] overflow-y-auto rounded-sm border border-[var(--line-strong)] bg-[#0d0d0d] p-2 shadow-[0_30px_80px_-20px_rgba(0,0,0,0.9)] scrollbar-surgical">
                      {loading ? <div className="flex justify-center py-8"><Loader2 className="h-5 w-5 animate-spin text-zinc-600" /></div>
                        ...
                        : filteredProblems.map((p) => (
                          <button key={p._id} onClick={() => { setSelectedProblem(p); setSearchQuery(''); setSearchOpen(false); }} className="flex w-full items-center gap-4 rounded-sm px-4 py-3 text-left transition-colors hover:bg-white/[0.05]">
            ...
            <div className="max-w-2xl space-y-16">
              <div>
                ...
                <input value={joinCode} onChange={(e) => setJoinCode(e.target.value.toUpperCase().slice(0, 6))} placeholder="A3F1B2" maxLength={6}
                  className="display w-full border-b border-[var(--line-strong)] bg-transparent pb-3 text-[clamp(51px,9.3vw,96px)] uppercase leading-none tracking-[0.12em] text-zinc-50 outline-none placeholder:text-zinc-800 focus:border-[var(--ember)]" />
              ...
              <div className="border-t border-[var(--line)] pt-10">
                <div className="display text-[28.8px] text-zinc-100">No code? Get matched.</div>
                ...
                {isMatchmaking
                  ? <button onClick={cancelMatchmaking} className="mt-6 flex items-center gap-3 rounded-sm border border-rose-400/40 px-6 py-3 font-mono text-[10.5px] uppercase tracking-[0.16em] text-rose-300 transition-colors hover:bg-rose-400/10"><Loader2 className="h-4 w-4 animate-spin" />Searching for an opponent… cancel</button>
                  : <button onClick={findMatch} disabled={!connected} className="mt-6 rounded-sm border border-[var(--line-strong)] px-6 py-3 font-mono text-[10.5px] uppercase tracking-[0.16em] text-zinc-300 transition-colors hover:border-[var(--ember)] hover:text-[var(--ember)] disabled:opacity-40">Find me a match</button>}
              </div>
            </div>
          )}
        </div>
