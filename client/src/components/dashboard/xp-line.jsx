  ...
  const data = useMemo(() => {
    ...
    switch (range) {
      ...
      case "30D":
        return xpSeries;
      case "90D":
        return [...xpSeries, ...xpSeries.map(v => v * 0.92), ...xpSeries.map(v => v * 0.8)].slice(0, 90);
      case "1Y":
        return Array.from({
          length: 52
        }, (_, i) => xpSeries[i * 3 % xpSeries.length]);
    }
  }, [range]);
  ...
  return <div className="relative flex h-full flex-col px-10 py-9">
      ...
      <div className="mb-8 flex items-end justify-between">
        ...
        <div className="flex items-center gap-1 border border-white/[0.06] p-[2px]">
          {RANGES.map(r => <button key={r} onClick={() => setRange(r)} className={`press ease-signature px-2.5 py-[3px] font-mono text-[10px] tracking-widest transition-colors ${r === range ? "bg-white/[0.05] text-zinc-100" : "text-zinc-600 hover:text-zinc-300"}`}>
              {r}
            </button>)}
        </div>
      </div>
