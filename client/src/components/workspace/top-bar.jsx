  ...
  return (
    <header className="relative flex h-12 items-center justify-between border-b border-[var(--line)] bg-[var(--background)] px-4">
      <div className="flex min-w-0 items-center gap-3.5">
        ...
        <span className="display max-w-[280px] truncate text-[19.2px] leading-none text-zinc-100">{problem?.title || 'Problem'}</span>
      ...
      <div className="absolute left-1/2 top-1/2 hidden -translate-x-1/2 -translate-y-1/2 items-center gap-3 md:flex" title="Probability that you've learned this skill — the tick marks 85%, where a skill counts as mastered">
        ...
        <span className="display text-[16px] leading-none tnum text-zinc-200">{bkt}<span className="text-[12px] text-zinc-600">%</span></span>
      </div>
      ...
      <div className="flex items-center gap-2 text-[12.5px]">
        ...
        <span className="ml-2 flex items-center gap-1.5 rounded-sm border border-[var(--line)] px-3 py-1 font-mono text-[10.5px] uppercase tracking-[0.16em]"><span className="h-1.5 w-1.5 animate-pulse rounded-full bg-[var(--ember)]" /><span className="tnum text-zinc-300">{formatHMS(elapsed)}</span></span>
      ...
      {showSettings && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={() => setShowSettings(false)}>
          <div className="w-full max-w-sm rounded-sm border border-[var(--line-strong)] bg-[#0d0d0d] p-8 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <div className="mb-6 flex items-center justify-between"><h3 className="display text-[25.6px] text-zinc-50">Editor</h3><button onClick={() => setShowSettings(false)} className="text-zinc-500 hover:text-zinc-200"><X className="h-5 w-5" /></button></div>
            <div className="mb-2 text-[13px] text-zinc-500">Keybindings</div>
            <div className="grid grid-cols-3 gap-2">
              {['standard', 'vim', 'emacs'].map((mode) => (
                <button key={mode} onClick={() => updateKeybindings(mode)} className={cn('rounded-sm border px-3 py-2 font-mono text-[10.5px] uppercase tracking-[0.16em] transition-colors', keybindings === mode ? 'border-[var(--ember)] bg-[var(--ember)]/10 text-[var(--ember-soft)]' : 'border-[var(--line)] text-zinc-400 hover:border-[var(--line-strong)]')}>{mode}</button>
              ))}
            </div>
