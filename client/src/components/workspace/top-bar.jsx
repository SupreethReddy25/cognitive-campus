...
import { ChevronRight, Command, ArrowLeft, Settings, X } from "lucide-react";
...
export function TopBar({ elapsed, stats }) {
  const { problem, id, result, keybindings, updateKeybindings } = useWorkspace();
  ...
  return <header className="relative flex h-[44px] items-center justify-between px-4 bg-[#0c0e12]" style={{ borderBottom: '1px solid rgba(74,124,89,0.08)' }}>
    ...
    <div className="flex items-center gap-3 font-mono text-[11px] tracking-wider">
      ...
      <span className="text-[10px] text-zinc-600 hidden sm:inline">{problem?.skillId?.name || 'DSA'}</span>
    ...
    <div className="flex items-center gap-3 font-mono text-[11px] tracking-wider">
      ...
      <button 
        ...
        className="press ease-signature flex items-center justify-center border border-transparent p-1.5 text-zinc-500 transition-colors duration-300 hover:text-zinc-300 rounded"
      >
    ...
    {showSettings && (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
        <div className="fade-in-up w-full max-w-sm rounded-xl border border-white/[0.08] bg-[#0d1117] p-6 shadow-2xl">
          <div className="flex items-center justify-between mb-6">
            <h3 className="text-[15px] font-semibold text-zinc-200">Editor Settings</h3>
            ...
            <div>
              ...
              <div className="grid grid-cols-3 gap-2">
                {['standard', 'vim', 'emacs'].map(mode => (
                  <button
                    key={mode}
                    onClick={() => updateKeybindings(mode)}
                    className={`rounded border px-3 py-2 text-[12px] capitalize transition-colors ${
                      keybindings === mode
              ...
              <p className="mt-3 text-[11px] text-zinc-500 leading-relaxed">
                Note: Vim/Emacs modes require installing additional Monaco plugins. Standard mode is default.
              </p>
            </div>
          </div>
        </div>
