...
export function ProfileHeatmap() {
  ...
  return <div className="relative">
      ...
      <div className="mb-2 grid grid-cols-12 gap-0 pl-6">
        {MONTHS.map(m => <span key={m} className="font-mono text-[9px] tracking-[0.22em] text-zinc-600">
            {m}
          </span>)}
      </div>
      ...
      <div className="flex gap-2">
        ...
        <div className="flex flex-1 gap-[3px]">
          {Array.from({
          length: weeks
        }).map((_, w) => <div key={w} className="flex flex-1 flex-col gap-[3px]">
              {Array.from({
            length: 7
          }).map((_, d) => {
            const idx = w * 7 + d;
            ...
            return <button key={d} type="button" onMouseEnter={() => setHover({
              week: w,
              day: d,
              count: v * 3
            })} onMouseLeave={() => setHover(null)} className={`aspect-square w-full border ${active ? "border-white/40" : "border-white/[0.06]"} transition-colors duration-150`} style={{
              backgroundColor: intensityBg(v)
      ...
      {/* tooltip */}
      {hover && <div className="pointer-events-none mt-3 flex items-center gap-3 font-mono text-[10px] tracking-[0.22em] text-zinc-500">
