...
export function LeaderboardView() {
  ...
  const Row = ({ r, i }) => (
    <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: Math.min(i * 0.02, 0.3) }}
      ...
      <span className="display text-[22.4px] leading-none tnum text-zinc-600">{r.rank}</span>
      <div className="flex min-w-0 items-center gap-3.5">
        <span className={cn('flex h-9 w-9 shrink-0 items-center justify-center rounded-full text-[11px] font-semibold', r.isCurrentUser ? 'bg-[var(--ember)] text-[#04130d]' : 'border border-[var(--line-strong)] text-zinc-400')}>{initials(r.name)}</span>
  ...
  return (
    <div>
          ...
          <div className="flex items-center gap-1">
            {[['global', 'Everyone'], ['college', 'My college']].map(([k, l]) => (
              <button key={k} onClick={() => setScope(k)} disabled={k === 'college' && !meta.collegeAvailable && scope !== 'college'} title={k === 'college' && !meta.collegeAvailable ? 'Choose your college in Profile to unlock' : ''}
                className={cn('rounded-sm px-4 py-2 text-[13px] transition-colors disabled:cursor-not-allowed disabled:opacity-40', scope === k ? 'bg-white/[0.08] text-zinc-50' : 'text-zinc-500 hover:text-zinc-200')}>{l}</button>
            ))}
          </div>
      ...
      {loading ? <div className="space-y-2">{Array.from({ length: 6 }).map((_, i) => <Skeleton key={i} className="h-14" />)}</div>
        : rows.length === 0 ? <div className="py-16 text-center"><div className="display text-[25.6px] italic text-zinc-500">{scope === 'college' ? 'Nobody else from your college is here yet.' : 'No students yet.'}</div></div>
        : (
          <>
            {podium.length === 3 && (
              <div className="mb-10 grid grid-cols-3 items-end gap-3 md:gap-8">
                {[1, 0, 2].map((idx) => { const r = podium[idx]; const st = PODIUM[idx]; return (
                  <motion.div key={r.userId} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.1 }} className="text-center">
                    <span className={cn('mx-auto flex h-14 w-14 items-center justify-center rounded-full text-[15px] font-semibold', r.isCurrentUser ? 'bg-[var(--ember)] text-[#04130d]' : 'border border-[var(--line-strong)] text-zinc-300')}>{initials(r.name)}</span>
                    <div className="mt-3 truncate text-[16px] font-medium text-zinc-100">{r.name}</div>
                    <div className="display mt-1 text-[24px] leading-none tnum" style={{ color: st.c }}>{r.xp.toLocaleString()}<span className="ml-1 text-[13px] text-zinc-600">xp</span></div>
                    <div className={cn('relative mx-auto mt-4 flex w-full items-start justify-center rounded-t-2xl border border-b-0 border-[var(--line-strong)] pt-3', st.h)} style={{ background: `linear-gradient(180deg, ${st.c}1f, transparent)` }}><span className="display text-[36.8px] leading-none" style={{ color: st.c }}>{idx + 1}</span></div>
                  </motion.div>
                ); })}
              </div>
            )}
            ...
            {!query && list.length > limit && <button onClick={() => setLimit((n) => n + 15)} className="mt-6 rounded-sm border border-[var(--line-strong)] px-6 py-2.5 font-mono text-[10.5px] uppercase tracking-[0.16em] text-zinc-400 hover:text-zinc-100">Show more</button>}
