            ...
            return (
              <motion.div key={s._id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: Math.min(i * 0.05, 0.3) }} onMouseEnter={() => setHover(s._id)}>
                <Link to={`/sheets/${s.slug}`} className="group relative grid items-end gap-x-10 gap-y-3 py-9 md:grid-cols-[64px_1fr_auto]">
                  ...
                  <span className="display hidden text-[38px] leading-none text-zinc-700 md:block">{String(i + 1).padStart(2, '0')}</span>
                  <div className="min-w-0">
                    <div className="text-[12.5px] text-zinc-500">{s.source || 'Curated'} · {s.totalProblems} problems · <span className="text-zinc-400">{s.availableProblems} live here</span></div>
                    <div className={cn('display mt-2 text-[clamp(34px,4.6vw,64px)] leading-[1] transition-all duration-300', active ? 'translate-x-2 text-[var(--ember)]' : 'text-zinc-100')}>{s.name}</div>
                  ...
                  <div className="flex items-end gap-6">
                    <div className="text-right"><div className="display text-[54px] leading-none tnum text-zinc-50">{Math.round(pct * 100)}<span className="text-[22px] text-zinc-600">%</span></div><div className="mt-1 text-[12px] text-zinc-500">{done} of {s.totalProblems}</div></div>
