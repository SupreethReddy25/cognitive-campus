...
export function WorkspaceShell() {
  ...
  const [results, setResults] = useState(() => Object.fromEntries(testCases.map(t => [t.id, {
    id: t.id,
    status: "idle"
  }])));
  const [running, setRunning] = useState(false);
  const [selectedTest, setSelectedTest] = useState(testCases[0].id);
  const timers = useRef([]);
  const runAll = useCallback(() => {
    ...
    setResults(prev => {
      ...
      for (const t of testCases) next[t.id] = {
        id: t.id,
        status: "running"
      };
      return next;
  ...
  const lighthouseLines = useMemo(() => lighthouse ? codeLines.filter(l => l.lighthouse).map(l => l.n) : [], [lighthouse]);
  ...
  return <div className="min-h-screen bg-background text-foreground">
      ...
      gridTemplateColumns: "48px 1fr"
        ...
        <div className="grid min-w-0" style={{
        gridTemplateRows: "44px 1fr 248px"
      }}>
          <TopBar elapsed={elapsed} stats={stats} />

          {/* Main workspace — 3 columns */}
          <div className="grid min-h-0 border-t border-white/[0.04]" style={{
          gridTemplateColumns: "380px 1fr 400px"
        }}>
            <LeftPane />
            <CenterPane highlightedLines={highlightedLines} lighthouseActive={lighthouse && citedLines.length === 0} activeLine={activeLine} onLineHover={setActiveLine} onRun={runAll} running={running} />
            <RightPane lighthouse={lighthouse} onLighthouseChange={setLighthouse} onCite={setCitedLines} />
          </div>

          <TestTiles running={running} results={results} onRun={runAll} selected={selectedTest} onSelect={setSelectedTest} stats={stats} />
        </div>
      </div>
    </div>;
}