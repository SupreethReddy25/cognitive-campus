...
import { mentorSeed } from "@/lib/problem-data";
...
export function RightPane({
  ...
  const [turns, setTurns] = useState(mentorSeed);
  ...
  const submit = () => {
    ...
    window.setTimeout(() => {
      setTurns(ts => [...ts, {
        role: "mentor",
        text: "Walk me through the invariant you would hold. What is true just before line 18 runs, and what must remain true just after?",
        cite: [15, 16, 18]
      }]);
      setComposing(false);
    }, 900);
  ...
  return <aside className="relative flex min-h-0 flex-col border-l border-white/[0.04]">
      ...
      <div ref={scrollerRef} className="min-h-0 flex-1 overflow-y-auto scrollbar-surgical">
        <div className="space-y-6 px-6 py-5">
          {turns.map((t, i) => <Turn key={i} turn={t} index={i} onCite={onCite} />)}
      ...
      <div className="shrink-0 border-t border-white/[0.04] px-3 py-2.5">
        ...
        }} placeholder="Ask the mentor…" className="flex-1 bg-transparent font-sans text-[12.5px] tracking-tight text-zinc-200 placeholder:text-zinc-600 focus:outline-none" />
...
function Turn({
  ...
  return <div className="stagger-in" style={{
    animationDelay: `${Math.min(index, 4) * 60}ms`
      ...
      <div className={isYou ? "border-l border-white/[0.08] pl-3 font-sans text-[13px] leading-relaxed text-zinc-300" : "font-sans text-[13.5px] leading-[1.55] text-zinc-200"}>
        {turn.text}
      </div>
...
function Dot({
  ...
  return <span className="tick-shimmer inline-block h-1 w-1 rounded-full bg-zinc-500" style={{
...
function MasteryDelta() {
  ...
  return <div className="shrink-0 border-t border-white/[0.04] px-3 py-2">
      ...
      <div className="mt-1.5 grid grid-cols-3 gap-1.5">
        {Object.entries(delta).map(([k, v]) => <div key={k} className="space-y-1">
            <div className="h-[2px] w-full bg-white/[0.05]">
              <div className="h-full bg-[var(--signal)]/80 transition-[width] duration-700" style={{
            width: `${Math.min(100, v * 400)}%`
          }} />
            </div>
