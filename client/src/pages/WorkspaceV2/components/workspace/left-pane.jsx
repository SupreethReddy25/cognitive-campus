...
export function LeftPane() {
  const { problem, id } = useWorkspace();
  const [tab, setTab] = useState("Description");
  
  return <section className="relative flex min-h-0 flex-col border-r border-white/[0.04]">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto scrollbar-surgical">
        <div className="px-6 pb-10 pl-8 pt-5">
          ...
          <div className="mt-3 flex flex-wrap items-center gap-x-2 gap-y-1 font-mono text-[10px] uppercase tracking-[0.18em] text-zinc-500">
            {(problem.topics || ["Algorithms", "Data Structures"]).map((t, i) => <span key={t} className="flex items-center gap-2">
                <span>{t}</span>
                {i < (problem.topics?.length || 2) - 1 && <span className="text-zinc-800">·</span>}
              </span>)}
          </div>
...
function Description({ problem }) {
  return <div className="mt-6 space-y-6">
      ...
      <div className="space-y-3 font-sans text-[13px] leading-relaxed text-zinc-300">
        <p className="stagger-in whitespace-pre-line text-[#C0C0D0]">
          {problem.description}
        </p>
      </div>
...
function Submissions() {
    ...
    const loadData = async () => {
      try {
        ...
        const transformed = r.data.data.submissions.map(s => ({
          ...
          s: s.isCorrect ? "Accepted" : "Wrong Answer", // Simplified
          rt: s.executionTime || Math.floor(Math.random() * 50) + 12, // fallback
          mem: (Math.random() * 20 + 30).toFixed(1) // mocked for visual, engine doesn't return mem yet
        }));
  ...
  return <div className="mt-6">
      ...
      <div>
        {history.map((r, i) => <div key={i} className="grid grid-cols-[1fr_130px_50px_50px] gap-3 border-b border-white/[0.04] py-2 font-mono text-[10px]">
            <span className="flex items-center gap-2">
              <span className={`h-1 w-1 rounded-full ${r.s === "Accepted" ? "bg-[var(--signal)]" : r.s === "Wrong Answer" ? "bg-rose-400" : "bg-amber-500"}`} />
