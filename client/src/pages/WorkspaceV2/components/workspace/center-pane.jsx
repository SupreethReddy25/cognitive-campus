...
import { ChevronDown, Command, Eraser, Play, RotateCcw, Send, Wand2, Check } from "lucide-react";
const LANGS = ["TypeScript", "Python", "Go", "Rust", "C++"];
export function CenterPane({
  ...
  return <section className="relative flex min-h-0 flex-col">
      ...
      {/* Monaco stage */}
      <div className="relative min-h-0 flex-1 overflow-auto scrollbar-surgical" onMouseLeave={() => onLineHover(null)}>
        ...
        <div className="relative font-mono text-[12.5px] leading-[1.7]">
          ...
          return <div key={line.n} onMouseEnter={() => onLineHover(line.n)} className={`group relative grid grid-cols-[56px_1fr] items-start transition-colors duration-200 ${isHl ? "lighthouse-line" : ""} ${isActive ? "bg-white/[0.015]" : ""}`}>
                <span className={`select-none pr-4 pt-[1px] text-right tabular-nums ${isActive ? "text-zinc-400" : isHl ? "text-[var(--signal)]/80" : "text-zinc-700"}`}>
                  {line.n}
                </span>
                <span className="whitespace-pre pr-8">
                  {line.tokens.map((tok, i) => <span key={i} className={tok.c}>
                      {tok.t}
                    </span>)}
                  {isActive && <span className="caret-blink ml-[1px] inline-block h-[14px] w-[1.5px] translate-y-[2px] bg-zinc-300 align-middle" />}
                </span>

                {isHl && <span className="pointer-events-none absolute right-5 top-1/2 -translate-y-1/2 font-mono text-[9px] tracking-widest text-[var(--signal)]/70">
                    ◂ PIVOT
                  </span>}
              </div>;
        })}
        ...
        {lighthouseActive && highlightedLines.length > 0 && <div className="pointer-events-none absolute right-8 top-4 z-[2] border border-[var(--signal)]/30 bg-[var(--signal)]/5 px-3 py-1.5 font-mono text-[9px] tracking-[0.2em] text-[var(--signal)] backdrop-blur-sm">
      ...
      <div className="pointer-events-none absolute inset-x-0 bottom-3 z-[3] flex justify-center">
        <div className="pointer-events-auto ring-hair flex items-center gap-1 border border-white/[0.08] bg-[#0a0a0a]/80 p-1 backdrop-blur-md">
          ...
          <button onClick={onRun} disabled={running} className={`press ease-signature flex items-center gap-2 px-3 py-1.5 font-mono text-[11px] tracking-widest transition-colors duration-300 ${running ? "text-zinc-600" : "text-zinc-200 hover:bg-white/[0.04]"}`}>
...
function LangSelector({
  ...
  return <div className="relative">
      ...
      {open && <div className="fade-in-up absolute right-0 top-full z-10 mt-1 w-44 border border-white/[0.08] bg-[#0c0c0c] p-1">
          {LANGS.map(l => <button key={l} onClick={() => {
        onSelect(l);
        setOpen(false);
...
function GhostBtn({
  ...
  return <button aria-label={label} className="press ease-signature flex items-center gap-1.5 px-2.5 py-1.5 font-mono text-[11px] tracking-widest text-zinc-500 transition-colors duration-300 hover:bg-white/[0.03] hover:text-zinc-200">
      {children}
    </button>;
}